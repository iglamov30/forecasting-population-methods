...
def main():
    ...
    decile_pi.to_csv(os.path.join(DATA_DIR, "decile_intervals.csv"), index=False)

    decile_mape = mape_by_decile(results, panel)
    decile_mape.to_csv(os.path.join(DATA_DIR, "decile_mape.csv"), index=False)

    mb = model_based_intervals(panel)
    mb.to_parquet(os.path.join(DATA_DIR, "model_based_intervals.parquet"), index=False)
    ...
    cov.to_csv(os.path.join(DATA_DIR, "interval_coverage.csv"), index=False)
