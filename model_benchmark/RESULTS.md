...
(see `README.md`); the underlying CSVs are in `model_benchmark/data/`.

## What is NOT established (read this before the tables)
...
- **The Phase 4 coverage numbers are in-sample.** The empirical intervals
...
- **The 250,000-population filter yielded 96 cities, not the ~800 originally
  anticipated** (flagged before Phase 0 was run and confirmed after: ~800
  is closer to ACS1's own ~65,000-population publication threshold across
  all places, not the count of cities above 250,000).
- **The Phase 5 pooled model result argues against our own prior going in.**
  ...
  but it does NOT beat simply running per-city ARIMA(1,1,1) (Phase 2's
  winner) at any horizon. Both things being true at once is the actual
  finding -- see Phase 5 below for the likely explanation.

## Phase 2: model-by-horizon backtest (expanding window, h=1..5)
...
## Phase 3: ensembles vs. best-model selection
...
## Phase 4: empirical vs. model-based 90% prediction intervals
...
horizon**: 82% coverage at h=1 collapsing to 43% at h=5, against a 90%
...
## Phase 5: pooled/global growth model vs. per-city local estimation
...
This is the headline result of Phase 5, and it complicates our own opening
